...
class SeqEntityScore(object):
    ...
    def update(self, label_paths, pred_paths):
        ...
        for label_path, pre_path in zip(label_paths, pred_paths):
            label_entities = entity_tag_extractor.get_entities(label_path, self.id2label,self.markup)
            pre_entities = entity_tag_extractor.get_entities(pre_path, self.id2label,self.markup)
...
class SpanEntityScore(object):
    ...
    def update(self, true_subject_batch, pred_subject_batch):
        for idx in range(len(true_subject_batch)):

            true_subject = true_subject_batch[idx]
            pred_subject = pred_subject_batch[idx]
