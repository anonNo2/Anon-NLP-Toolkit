...
class T5AQGenScore(T5GenScore):
    ...
    def get_machine_metric_datas(self,model,output_dir,data_type):
        ...
        for single_para_idx in tqdm(range(len(raw_paras))):
            ...
            splitors = single_para.split('\t')
            questions = [i for i in splitors[0].split('@')]
            answer = splitors[1]
            for quest_item in questions:
                gen_text_tok = self.generate_text_by_input(quest_item, [], model_to_gen, self.context.tokenizer)
                gen_text = "".join(gen_text_tok)
                results.append({'ori':answer,'gen':gen_text})
