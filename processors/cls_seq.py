...
class AutoClsProcessor(DataProcessor):
    ...
    def get_examples_common(self,data_dir,tag):
        if f'{tag}.json' in os.listdir(data_dir):
            return self._create_cls_examples(self._read_cls_json(os.path.join(data_dir, f"{tag}.json")), tag)
        elif f'{tag}.xls' in os.listdir(data_dir) :
            return self._create_cls_examples(self._read_cls_xls(os.path.join(data_dir, f"{tag}.xls")), tag)
        elif f'{tag}.xlsx' in os.listdir(data_dir):
            return self._create_cls_examples(self._read_cls_xls(os.path.join(data_dir, f"{tag}.xlsx")), tag)
        elif f'{tag}.csv' in os.listdir(data_dir):
            return self._create_cls_examples(self._read_cls_csv(os.path.join(data_dir, f"{tag}.csv")), tag)
    ...
    def get_train_examples(self, data_dir):
        return ClsEnhancer(self.args).augments_data(self.get_examples_common(data_dir, "train"))
    ...
    def _create_cls_examples(self, lines, set_type):
